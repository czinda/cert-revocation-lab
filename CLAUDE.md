...
## idm-ci / mrack Deployment

The `idm-ci/` directory provides mrack-based provisioning and Jenkins Job Builder definitions for deploying the lab on remote machines. Single-host topology (all ~25 containers on one Fedora machine). Supports any mrack provider via `MRACK_PROVIDER` env var (default: `beaker`).

```bash
cd idm-ci
./scripts/prepare-hosts.sh                        # Provision Fedora (mrack up)
...
./scripts/shutdown-hosts.sh                        # Return machine (always run this)
```

**Machine specs**: 16 vCPU, 64 GB RAM, 100 GB disk (`hostRequires` in `config/provisioning-config.yaml`). **Target OS**: Fedora latest (`Fedora%` distro pattern). **Jenkins jobs**: `certlab-deploy` (manual) and `certlab-nightly` (cron 2 AM, full regression). See `idm-ci/README.md` for full documentation.
