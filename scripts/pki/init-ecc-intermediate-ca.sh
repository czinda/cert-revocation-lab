#!/bin/bash
#
# init-ecc-intermediate-ca.sh - Initialize the Dogtag ECC Intermediate CA
...
phase1_generate_csr() {
    ...
    export_pki_env
    prepare_config "${CONFIG_DIR}/ecc-intermediate-ca-step1.cfg" /tmp/step1.cfg
    pkispawn -s CA -f /tmp/step1.cfg --skip-configuration -v
    rm -f /tmp/step1.cfg

    log_info "CSR generated: $CSR_FILE"
    print_sign_action "$CSR_FILE" "$SIGNED_CERT" "dogtag-ecc-root-ca" "$ROOT_CA_URL" "caCACert"
}
...
phase2_install_cert() {
    ...
    [ -f "${CERTS_DIR}/root-ca.crt" ] || { log_error "ECC Root CA cert not found"; return 1; }

    export_pki_env
    prepare_config "${CONFIG_DIR}/ecc-intermediate-ca-step2.cfg" /tmp/step2.cfg
    ...
    echo "Web UI:      https://ecc-intermediate-ca.cert-lab.local:8443/ca"
    echo ""
    echo "Next: podman exec -it dogtag-ecc-iot-ca /scripts/init-ecc-iot-ca.sh"
}

init_ca() {
    print_header "Initializing Dogtag ECC Intermediate CA (P-384)"
    mkdir -p "$CERTS_DIR"
    ...
    wait_for_ca "ECC Root CA" "$ROOT_CA_URL" "${CERTS_DIR}/root-ca.crt"
