#!/bin/bash
#
# init-pq-intermediate-ca.sh - Initialize the Dogtag PQ Intermediate CA
...
phase2_install_cert() {
    ...
    [ -f "${CERTS_DIR}/root-ca.crt" ] || { log_error "PQ Root CA cert not found"; return 1; }
...
init_ca() {
    ...
    wait_for_ca "PQ Root CA" "$ROOT_CA_URL" "${CERTS_DIR}/root-ca.crt"
