#!/bin/bash
#
# init-pq-intermediate-ca.sh - Initialize the Dogtag PQ Intermediate CA (ML-DSA-87)
...
CSR_FILE="${CERTS_DIR}/pq-intermediate-ca.csr"
SIGNED_CERT="${CERTS_DIR}/pq-intermediate-ca-signed.crt"
CA_CERT="${CERTS_DIR}/pq-intermediate-ca.crt"

phase1_generate_csr() {
    log_info "Phase 1: Generating ML-DSA-87 CSR..."
    ...
    export_pki_env
    prepare_config "${CONFIG_DIR}/pq-intermediate-ca-step1.cfg" /tmp/step1.cfg
    pkispawn -s CA -f /tmp/step1.cfg --skip-configuration -v
    rm -f /tmp/step1.cfg

    log_info "CSR generated: $CSR_FILE"
    print_sign_action "$CSR_FILE" "$SIGNED_CERT" "dogtag-pq-root-ca" "$ROOT_CA_URL" "caCACert"
}
...
phase2_install_cert() {
    ...
    [ -f "${CERTS_DIR}/pq-root-ca.crt" ] || { log_error "PQ Root CA cert not found"; return 1; }

    export_pki_env
    prepare_config "${CONFIG_DIR}/pq-intermediate-ca-step2.cfg" /tmp/step2.cfg
    ...
    create_chain "${CERTS_DIR}/pq-ca-chain.crt" "${CERTS_DIR}/pq-root-ca.crt" "$CA_CERT"
    verify_cert "$CA_CERT" "${CERTS_DIR}/pq-root-ca.crt"

    print_header "PQ Intermediate CA Initialization Complete"
    echo "Algorithm:   ML-DSA-87 (NIST FIPS 204)"
    echo "Certificate: $CA_CERT"
    echo "CA Chain:    ${CERTS_DIR}/pq-ca-chain.crt"
    echo "Web UI:      https://pq-intermediate-ca.cert-lab.local:8443/ca"
    echo ""
    echo "Next: podman exec -it dogtag-pq-iot-ca /scripts/init-pq-iot-ca.sh"
}

init_ca() {
    print_header "Initializing Dogtag PQ Intermediate CA (ML-DSA-87)"
    mkdir -p "$CERTS_DIR"
    ...
    wait_for_ca "PQ Root CA" "$ROOT_CA_URL" "${CERTS_DIR}/pq-root-ca.crt"
