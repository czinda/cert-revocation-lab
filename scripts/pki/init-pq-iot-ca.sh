#!/bin/bash
#
# init-pq-iot-ca.sh - Initialize the Dogtag PQ IoT Sub-CA (ML-DSA-87)
...
CSR_FILE="${CERTS_DIR}/pq-iot-ca.csr"
SIGNED_CERT="${CERTS_DIR}/pq-iot-ca-signed.crt"
CA_CERT="${CERTS_DIR}/pq-iot-ca.crt"
CA_CHAIN="${CERTS_DIR}/pq-ca-chain.crt"
...
phase2_install_cert() {
    ...
    create_chain "${CERTS_DIR}/pq-iot-ca-chain.crt" "$CA_CHAIN" "$CA_CERT"
    ...
    echo "CA Chain:    ${CERTS_DIR}/pq-iot-ca-chain.crt"
...
init_ca() {
    ...
    wait_for_ca "PQ Intermediate CA" "$INTERMEDIATE_CA_URL" "${CERTS_DIR}/pq-intermediate-ca.crt"
