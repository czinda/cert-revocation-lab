#!/bin/bash
#
# init-pq-root-ca.sh - Initialize the Dogtag PQ Root CA (self-signed)
...
init_ca() {
    print_header "Initializing Dogtag PQ Root CA (ML-DSA-87)"
    mkdir -p "$CERTS_DIR"
    ...
    export_admin_creds "$PKI_INSTANCE" "pq-root"
