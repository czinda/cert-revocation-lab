...
setup_directories() {
    ...
    mkdir -p data/certs/rsa
    mkdir -p data/certs/pq
    mkdir -p data/pki/{root,intermediate,iot}
    mkdir -p data/pki/pq/{root,intermediate,iot}
...
setup_networks() {
    ...
    for net_name in "${!NETWORKS[@]}"; do
        ...
        [[ "$net_name" == "pki-net" || "$net_name" == "pki-pq-net" || "$net_name" == "freeipa-net" ]] && is_rootful=true
...
setup_volumes() {
    ...
    # Rootful volumes (PKI compose - RSA and PQ)
    ROOTFUL_VOLUMES=(
        "ds-root-data"
...
# Post-Quantum PKI (ML-DSA-87)
172.27.0.11 pq-intermediate-ca.cert-lab.local pq-intermediate-ca
172.27.0.12 pq-root-ca.cert-lab.local pq-root-ca
172.27.0.13 pq-iot-ca.cert-lab.local pq-iot-ca
172.27.0.14 ds-pq-root.cert-lab.local ds-pq-root
172.27.0.15 ds-pq-intermediate.cert-lab.local ds-pq-intermediate
172.27.0.16 ds-pq-iot.cert-lab.local ds-pq-iot
EOF
...
clean_start() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        # Clean rootful RSA PKI containers
        if [ -f pki-compose.yml ]; then
            log_info "Cleaning RSA PKI containers (requires sudo)..."
            if is_running_as_root; then
        ...
        # Clean rootful PQ PKI containers
        if [ -f pki-pq-compose.yml ]; then
            log_info "Cleaning PQ PKI containers (requires sudo)..."
            if is_running_as_root; then
                podman-compose -f pki-pq-compose.yml down -v 2>/dev/null || true
                podman volume prune -f 2>/dev/null || true
            else
                sudo podman-compose -f pki-pq-compose.yml down -v 2>/dev/null || true
...
start_pki_hierarchy() {
    ...
    if [ "$all_running" = true ]; then
        log_success "All RSA PKI containers already running, skipping initialization"
        return
    fi
...
# Phase 4b: Start and Initialize Post-Quantum PKI Hierarchy
start_pq_pki_hierarchy() {
    log_phase "Phase 4b: Starting Post-Quantum PKI Infrastructure (ML-DSA-87)"

    # Check if PQ PKI compose file exists
    if [ ! -f pki-pq-compose.yml ]; then
        log_warn "pki-pq-compose.yml not found. Skipping PQ PKI initialization."
        return
    fi
    ...
    # Start PQ PKI containers with rootful podman
    if is_running_as_root; then
    ...
    log_info "Initializing Post-Quantum PKI hierarchy (ML-DSA-87)..."
    ...
    log_success "Post-Quantum PKI hierarchy initialized"
}
...
print_summary() {
    ...
    echo ""
    echo "  ML-DSA-87 PKI (Post-Quantum):"
    ...
    echo ""
    echo "Dual PKI Hierarchy:"
    echo ""
    echo "  RSA-4096 (Traditional):          ML-DSA-87 (Post-Quantum):"
    echo "  Root CA (self-signed)            PQ Root CA (self-signed)"
    echo "    +-- Intermediate CA              +-- PQ Intermediate CA"
    echo "        +-- IoT Sub-CA                   +-- PQ IoT Sub-CA"
    echo ""
    echo "RSA Certificates:           PQ Certificates:"
    echo "  data/certs/rsa/             data/certs/pq/"
    echo "  root-ca.crt                 pq-root-ca.crt"
    echo "  intermediate-ca.crt         pq-intermediate-ca.crt"
    echo "  iot-ca.crt                  pq-iot-ca.crt"
    echo "  ca-chain.crt                pq-ca-chain.crt"
    echo ""
    echo "Testing:"
    ...
    echo "  sudo podman-compose -f pki-pq-compose.yml logs -f <service-name>"
    echo ""
    echo -e "${GREEN}========================================================================${NC}"
}
...
quick_start() {
    ...
    # Start RSA PKI containers from pki-compose.yml (rootful - has initialized data)
    if [ -f pki-compose.yml ]; then
        local rsa_all_running=true
        for ctr in ds-root ds-intermediate ds-iot dogtag-root-ca dogtag-intermediate-ca dogtag-iot-ca; do
            if is_rootful_running "$ctr"; then
                log_success "$ctr is already running"
            else
                rsa_all_running=false
            fi
        done

        if [ "$rsa_all_running" = true ]; then
            log_success "All RSA PKI containers already running"
        else
            log_info "Starting RSA PKI containers (from pki-compose.yml)..."
            if [ "$RUNNING_AS_ROOT" = true ]; then
    ...
    # Start PQ PKI containers from pki-pq-compose.yml (rootful - has initialized data)
    if [ -f pki-pq-compose.yml ]; then
        local pq_all_running=true
        for ctr in ds-pq-root ds-pq-intermediate ds-pq-iot dogtag-pq-root-ca dogtag-pq-intermediate-ca dogtag-pq-iot-ca; do
            if is_rootful_running "$ctr"; then
                log_success "$ctr is already running"
            else
                pq_all_running=false
            fi
        done

        if [ "$pq_all_running" = true ]; then
            log_success "All PQ PKI containers already running"
        else
            log_info "Starting PQ PKI containers (from pki-pq-compose.yml)..."
            if [ "$RUNNING_AS_ROOT" = true ]; then
                podman-compose -f pki-pq-compose.yml up -d 2>/dev/null || {
                    log_warn "Failed to start PQ PKI containers"
                }
            else
                sudo podman-compose -f pki-pq-compose.yml up -d 2>/dev/null || {
                    log_warn "Failed to start PQ PKI containers. Try: sudo podman-compose -f pki-pq-compose.yml up -d"
                }
            fi

            # Start the PQ PKI servers inside containers
            sleep 5
    ...
    # Show status
    echo ""
    log_info "Checking RSA PKI status..."
    ...
    echo ""
    log_info "Checking PQ PKI status (ML-DSA-87)..."
    curl -sk https://localhost:8453/ca/admin/ca/getStatus 2>/dev/null | grep -q "running" && echo "  PQ Root CA: running" || echo "  PQ Root CA: not responding"
    curl -sk https://localhost:8454/ca/admin/ca/getStatus 2>/dev/null | grep -q "running" && echo "  PQ Intermediate CA: running" || echo "  PQ Intermediate CA: not responding"
    curl -sk https://localhost:8455/ca/admin/ca/getStatus 2>/dev/null | grep -q "running" && echo "  PQ IoT CA: running" || echo "  PQ IoT CA: not responding"
